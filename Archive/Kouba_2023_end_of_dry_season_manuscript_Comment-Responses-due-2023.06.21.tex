...
\CommentTok{\# Between approximately 100 and 140 cfs there is a plateau: t}
\CommentTok{\# }
\CommentTok{\# }
\CommentTok{\# calc\_dQdP = function(P, Q, dates, lag\_days = 1, }
\CommentTok{\#                      dry\_season\_day1 = yday(as.Date("2001{-}09{-}01")),}
\CommentTok{\#                      result="dQdP")\{}
\CommentTok{\#   P\_lagged\_forward = c(rep(NA, lag\_days), P[1:(length(P){-}lag\_days)])}
\CommentTok{\#   Q\_lagged\_forward = c(rep(NA, lag\_days), Q[1:(length(Q){-}lag\_days)])}
\CommentTok{\#   }
\CommentTok{\#     \# take out annual fenceposts {-} first x days of each year\textquotesingle{}s dry season (which in this tabular arrangement have lagged values from the preceding year)}
\CommentTok{\#   dates\_julian = yday(dates)}
\CommentTok{\#   Q[dates\_julian \textless{} (lag\_days+dry\_season\_day1) \& dates\_julian \textgreater{}= dry\_season\_day1] = NA }
\CommentTok{\#   P[dates\_julian \textless{} (lag\_days+dry\_season\_day1) \& dates\_julian \textgreater{}= dry\_season\_day1] = NA }
\CommentTok{\# }
\CommentTok{\#   dP = P {-} P\_lagged\_forward \# real time cum. precip. record minus cum. ppt from x days ago}
\CommentTok{\#   dQ = Q {-} Q\_lagged\_forward \# real time flow record minus flow from x days ago}
\CommentTok{\#   dQdP = dQ/dP}
\CommentTok{\#   if(result=="dQdP")\{return(dQdP)\}}
\CommentTok{\#   if(result=="dQ")\{return(dQ)\}}
\CommentTok{\# \}}
\CommentTok{\# }
\CommentTok{\# }
\CommentTok{\# \# calc\_dQdPs\_multiple\_lags = function(fj3d, max\_lag\_days=6)\{}
\CommentTok{\# \#   starting\_ncol = ncol(fj3d)}
\CommentTok{\# \#   fj3d[,(starting\_ncol+1):(starting\_ncol+max\_lag\_days)] = NA}
\CommentTok{\# \#   colnames(fj3d)[(starting\_ncol+1):(starting\_ncol+max\_lag\_days)] = paste0("dQdP\_lag",1:max\_lag\_days)}
\CommentTok{\# \#   for(lag\_days in 1:max\_lag\_days)\{}
\CommentTok{\# \#     result\_colname = paste0("dQdP\_lag",lag\_days)}
\CommentTok{\# \#     fj3d[,starting\_ncol+lag\_days] = calc\_dQdP(P = fj3d$cum\_ppt, }
\CommentTok{\# \#                                               Q = fj3d$Flow, }
\CommentTok{\# \#                                               dates = fj3d$Date, }
\CommentTok{\# \#                                               lag\_days = lag\_days)}
\CommentTok{\# \#   \}}
\CommentTok{\# \#   return(fj3d)}
\CommentTok{\# \# \}}
\CommentTok{\# }
\CommentTok{\# }
\CommentTok{\# calc\_cum\_dQdPs\_multiple\_lags = function(fj3d, max\_lag\_days=6)\{}
\CommentTok{\#   starting\_ncol = ncol(fj3d)}
\CommentTok{\#   fj3d[,(starting\_ncol+1):(starting\_ncol+max\_lag\_days)] = NA}
\CommentTok{\#   colnames(fj3d)[(starting\_ncol+1):(starting\_ncol+max\_lag\_days)] = paste0("dQdP\_lag",1:max\_lag\_days)}
\CommentTok{\#   for(lag\_days in 1:max\_lag\_days)\{}
\CommentTok{\#     result\_colname = paste0("dQdP\_lag",lag\_days)}
\CommentTok{\#     fj3d[,starting\_ncol+lag\_days] = calc\_dQdP(P = fj3d$cum\_ppt, }
\CommentTok{\#                                               Q = fj3d$cum\_flow\_m3, }
\CommentTok{\#                                               dates = fj3d$Date, }
\CommentTok{\#                                               lag\_days = lag\_days)}
\CommentTok{\#   \}}
\CommentTok{\#   return(fj3d)}
...
\CommentTok{\# }
\CommentTok{\# }
\CommentTok{\# dQdP\_vs\_flow = function(lag\_days = 1, }
\CommentTok{\#                         storm\_cutoff = 0.5*25.4)\{ \# half inch to mm}
\CommentTok{\# }
\CommentTok{\#   \# Change in flow in X days}
\CommentTok{\#   \# q1 = c(fj3d$Flow[1:(nrow(fj3d){-}lag\_days)], rep(NA, lag\_days))}
\CommentTok{\#   q1 = fj3d$cum\_flow\_m3 \# $Flow}
\CommentTok{\#   q2 = c(rep(NA, lag\_days), fj3d$cum\_flow\_m3[1:(nrow(fj3d){-}lag\_days)])}
\CommentTok{\#   \# Change in cumulative precip in X days}
\CommentTok{\#   \# p1 = c(fj3d$cum\_ppt[1:(nrow(fj3d){-}lag\_days)], rep(NA, lag\_days))}
\CommentTok{\#   p1 = fj3d$cum\_ppt}
\CommentTok{\#   \# p2 the cumulative precip vector adjusted forward by x days}
\CommentTok{\#   p2 = c(rep(NA, lag\_days), fj3d$cum\_ppt[1:(nrow(fj3d){-}lag\_days)])}
\CommentTok{\# }
\CommentTok{\#   \# take out annual fenceposts {-} first x days of each year (which are just values from the preceding year)}
\CommentTok{\#   fj3d$julian = yday(fj3d$Date)}
\CommentTok{\#   adj\_wy\_day1 = yday(as.Date("2001{-}09{-}01"))}
\CommentTok{\#   q1[fj3d$julian \textless{} (lag\_days+adj\_wy\_day1) \& fj3d$julian \textgreater{}= adj\_wy\_day1] = NA }
\CommentTok{\#   p1[fj3d$julian \textless{} (lag\_days+adj\_wy\_day1) \& fj3d$julian \textgreater{}= adj\_wy\_day1] = NA }
\CommentTok{\#   }
\CommentTok{\#   \#calculate dQ (the change in flow in x days) and dP (the change in cum. precip. in x days)}
\CommentTok{\#   fj3d$q1=q1; fj3d$q2=q2; fj3d$p1=p1; fj3d$p2 = p2}
\CommentTok{\#   fj3d$dQ = q1{-}q2 \# time{-}true record minus the record from x days ago}
\CommentTok{\#   fj3d$dP = p1{-}p2}
\CommentTok{\#   \# Calculate dQdP}
\CommentTok{\#   fj3d$dQdP = fj3d$dQ/fj3d$dP}
\CommentTok{\#   }
\CommentTok{\#   \# hist(fj3d$dQdP,xlim = c({-}1000,1000), breaks = 300)}
\CommentTok{\#   }
\CommentTok{\#   storm\_index = fj3d$ppt \textgreater{}= storm\_cutoff}
\CommentTok{\#   \# adjust for smoothing days}
\CommentTok{\#   \# storm\_index\_delayed = c(rep(NA,lag\_days),}
\CommentTok{\#   \#                         storm\_index[1:(length(storm\_index){-}lag\_days)])}
\CommentTok{\#   fj3d\_storm = fj3d[storm\_index,]}
\CommentTok{\#   }
\CommentTok{\#   \# summary(fj3d\_storm$dQdP,na.rm=T)}
\CommentTok{\#   plot(fj3d\_storm$dQdP, fj3d\_storm$cum\_flow\_m3, xlim = c({-}1*10\^{}3, 10\^{}3), }
\CommentTok{\#        pch = 19, col = rgb(0,0,0,.05), log = "y",}
\CommentTok{\#        main = paste("d\_Flow d\_Precip, \textbackslash{}n smoothing", lag\_days, "day(s), storm cutoff",storm\_cutoff, "mm, storms only"))}
\CommentTok{\#   grid()}
\CommentTok{\#   abline(h=100, col = "brown", lwd=2)}
\CommentTok{\#   }
\CommentTok{\#     plot(fj3d\_storm$dP, fj3d\_storm$dQ, \#xlim = c({-}1, 10\^{}2), }
\CommentTok{\#        pch = 19, col = rgb(0,0,0,.1), log = "y",}
\CommentTok{\#        main = paste("d\_Precip vs d\_Flow, \textbackslash{}n smoothing", lag\_days, "day(s), storm cutoff",storm\_cutoff, "mm, storms only"))}
\CommentTok{\#   grid()}
\CommentTok{\#   abline(h=100, col = "brown", lwd=2)}
\CommentTok{\# }
\CommentTok{\#   \# plot(fj3d$dQdP, fj3d$Flow, xlim = c({-}1*10\^{}3, 10\^{}3), }
\CommentTok{\#   \#      pch = 19, col = rgb(0,0,0,.05), log = "y",}
\CommentTok{\#   \#      main = paste("d\_Flow d\_Precip, \textbackslash{}n smoothing", lag\_days, "day(s), storm cutoff",storm\_cutoff, "mm, all days"))}
\CommentTok{\#   \# grid()}
\CommentTok{\#   \# abline(h=100, col = "brown", lwd=2)}
\CommentTok{\#   }
\CommentTok{\#   }
...
\CommentTok{\# computation steps: }
\CommentTok{\# calculate dQ and dP (Q\_i {-} Q\_i{-}1), and dQ/dP}
\CommentTok{\# don\textquotesingle{}t need days for which dQ is negative. Flow can be falling even on days with some rainfall if it\textquotesingle{}s coming down from a higher peak}
\CommentTok{\# try to skew for early in the wet season so we avoid snowmelt influence}
\CommentTok{\# filter for days that got more than 1/10th inch. Because small dPs can produce high dQ dP values that are not meaningful. }
\DocumentationTok{\#\#\# why 1/10th inch? make up some bullshit justification about refET in the winter?}


\CommentTok{\# 653 sq miles drainage area for Ft Jones gauge (per USGS https://waterdata.usgs.gov/monitoring{-}location/11519500/\#parameterCode=00060\&period=P7D)}
\CommentTok{\# 1 ft3/sec * 1m3/35 ft3 * 86400 sec/day / 1.7m sq meters * 1000mm/m = mm/day}



\CommentTok{\# }
\CommentTok{\#   \#summarize for each threshold}
\CommentTok{\# delta\_dqdp\_mean = aggregate(x = results$delta\_dQdP,}
\CommentTok{\#                             by = list(results$Q\_threshold\_m3day), FUN = mean,}
\CommentTok{\#                             na.rm=T,finite=T)$x}
\CommentTok{\# delta\_dqdp\_med = aggregate(x = results$delta\_dQdP,}
\CommentTok{\#                             by = list(results$Q\_threshold\_m3day), FUN = median, na.rm=T)$x}
\CommentTok{\#   \#   mean(results\_for\_wys$delta\_dqdp, na.rm=T)}
\CommentTok{\#   \# results\_for\_thresholds$delta\_dqdp\_med[threshold\_picker] = median(results\_for\_wys$delta\_dqdp, na.rm=T)}
\CommentTok{\#   \# results\_for\_thresholds$delta\_dqdp\_std[threshold\_picker] = sd(results\_for\_wys$delta\_dqdp, na.rm=T)}
\CommentTok{\# }
\CommentTok{\# plot(q\_thresh/cfs\_to\_m3day, delta\_dqdp\_mean / cfs\_to\_m3day)}
\CommentTok{\# grid()}
\CommentTok{\# }
\CommentTok{\# plot(q\_thresh/cfs\_to\_m3day,delta\_dqdp\_med / cfs\_to\_m3day)}
\CommentTok{\# grid()}

\CommentTok{\# for(yr in EoDS\_years)\{}
\CommentTok{\#   yr\_picker = results$year==yr}
\CommentTok{\#   thresholds = results$Q\_threshold\_m3day[yr\_picker]}
\CommentTok{\#   delta\_dqdp = results$delta\_dQdP[yr\_picker]}
\CommentTok{\# }
\CommentTok{\#   if(yr==EoDS\_years[1])\{}
\CommentTok{\#     plot(thresholds/cfs\_to\_m3day, delta\_dqdp/cfs\_to\_m3day, main = "Effect of dQ threshold value on runoff{-}rainfall \textbackslash{}n responses in end of dry and beginning of wet season",}
\CommentTok{\#          xlab = "dQ end{-}of{-}dry{-}season threshold (m3day)",}
\CommentTok{\#          ylab = "change in dQ/dP (wet season minus dry season)",}
\CommentTok{\#          ylim = range(results$delta\_dQdP,na.rm=T, finite=T)/cfs\_to\_m3day,}
\CommentTok{\#          type = "o", col = rgb(.2,.2,.2,.2))}
\CommentTok{\#   \} else \{}
\CommentTok{\#     lines(thresholds/cfs\_to\_m3day, delta\_dqdp/cfs\_to\_m3day, col = rgb(.2,.2,.2,.2), type = "o")}
\CommentTok{\#   \}}
\CommentTok{\# \}}


\CommentTok{\# thresh\_colors = sequential\_hcl(n=length(q\_thresh), palette = "Hawaii")}
\CommentTok{\# }
\CommentTok{\# for(i in 1:length(q\_thresh))\{}
\CommentTok{\#   thresh=q\_thresh[i]}
\CommentTok{\#   thresh\_picker = results$Q\_threshold\_m3day==thresh}
\CommentTok{\#   years = results$year[thresh\_picker]}
\CommentTok{\#   delta\_dqdp = results$delta\_dQdP[thresh\_picker]}
\CommentTok{\#   thresh\_col = thresh\_colors[i]}
\CommentTok{\#   }
\CommentTok{\#   if(thresh==q\_thresh[1])\{}
\CommentTok{\#     plot(years, delta\_dqdp/cfs\_to\_m3day, log="y",}
\CommentTok{\#          main = "Yearly pattern, by threshold, of runoff{-}rainfall \textbackslash{}n responses in end of dry and beginning of wet season",}
\CommentTok{\#          xlab = "Year",}
\CommentTok{\#          ylab = "change in dQ/dP (wet season minus dry season)",}
\CommentTok{\#          \# ylim = range(results$delta\_dQdP,na.rm=T, finite=T)[2]/cfs\_to\_m3day,}
\CommentTok{\#          col=thresh\_col,\# col = rgb(.2,.2,.2,.2),}
\CommentTok{\#          type = "o")}
\CommentTok{\#   \} else \{}
\CommentTok{\#     lines(years, delta\_dqdp/cfs\_to\_m3day, \#col = rgb(.2,.2,.2,.2), }
\CommentTok{\#           type = "o", col = thresh\_col, pch=3)}
\CommentTok{\#   \}}
\CommentTok{\# \}}

\CommentTok{\# if(write\_q\_thresh\_pdf)\{}
\CommentTok{\#   pdf(file = "First spill day viewer, all WYs\_120cfs.pdf", width = 8.5, height = 11)}
\CommentTok{\#   par(mfrow = c(2,1), mar = c(5,4,4,4)*1.1)}
\CommentTok{\#   for(yr in EoDS\_years)\{}
\CommentTok{\#     dQdP\_vs\_hydrograph(fj3d = fj3d, }
\CommentTok{\#                        storm\_cutoff = 6.35,}
\CommentTok{\#                        date\_1 = as.Date(paste0(yr,"{-}09{-}01")), }
\CommentTok{\#                        date\_2 = as.Date(paste0(yr+1,"{-}02{-}01")),}
\CommentTok{\#                        as\_png = F, }
\CommentTok{\#                        max\_lag\_days = 1,}
\CommentTok{\#                        Q\_spill\_threshold = 120) \#threshold in cfs}
\CommentTok{\#   \}}
\CommentTok{\#   }
\CommentTok{\#   dev.off()}
\CommentTok{\# \}}
