...
minimum 30-day dry season baseflow volume, \(V_{min,~30~days}\), which
occurs at the end of the dry season (September-October) in this
Mediterranean climate. The second metric is the cumulative
precipitation, starting Sept.~1st, necessary to bring the watershed to a
``full'' or ``spilling'' condition (i.e.~initiate the onset of wet
season storm- or baseflows) after the end of the dry season, referred to
here as \(P_{spill}\). As potential predictors of these two values, we
...
linear models could reproduce historic values of \(V_{min,~30~days}\)
and \(P_{spill}\) with an average model error (RMSE) of 1.4
